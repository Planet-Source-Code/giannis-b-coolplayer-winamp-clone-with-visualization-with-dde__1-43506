...
Option Explicit
Private Sub Check(C As CheckBox)
    C.Value = IIf(C.Value = 1, 1, 0)
End Sub
...
Private Sub cmdNone_Click()
    Call SelList(False, lstTypes)
End Sub

Private Sub cmdPlugAb_Click()

    If lstPlugs.Selected(0) = True Then
     Call File.AboutPlugin(CI.bTop)
    ElseIf lstPlugs.Selected(1) = True Then
     Call Vis.AboutPlugin(CI.bTop)
    End If
...
Private Sub sliT_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 1 Then Call SaveOptions
End Sub
Private Sub Tab_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
