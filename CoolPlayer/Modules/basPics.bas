...
Public Sub DrawVis()
    ...
    On Error Resume Next
    Call frmMn.Scope.PaintPicture(frmMn.Picture, 0, 0, 75, 14, 24, 44, 75, 14)
...
Public Sub OpenForFile(Frm As Form)
    ...
    Call AddFile(CD.FileName, CD.FileTitle, True, True)
...
Public Sub OpenForLoad(Frm As Form)
    ...
    End With
    Call DisableForms(True)
    Call LoadList(CD.FileName, False)
...
Public Sub OpenForSave(Frm As Form)
    ...
    End With
    Call DisableForms(True)
    Call Lst.saveList(CD.FileName, CD.FilterIndex, frmPl.l)
