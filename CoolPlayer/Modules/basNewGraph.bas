...
Public Sub LoadTracks()
    ...
    With frmPl.l.ListItems
     For i = 1 To .Count
      frmPro.lstP.AddItem .Item(i)
     Next i
    End With

    With frmPro.lstP
     For i = 1 To UBound(PL.Tr)
      If PL.Tr(i) <> 0 Then
       .Selected(PL.Tr(i) - 1) = True
       j = j + 1
      End If
     Next
     frmPro.lblSl.Caption = j & " from " & .ListCount & " items, " & _
                            CLng((j * 100) / .ListCount) & "% of list."
     ReDim PL.Tr(0 To .ListCount - 1)
    End With
...
End Sub
Public Sub SelList(Sel As Boolean, l As ListBox)
...
End Sub
Public Sub LoadfrmSet()

    With frmSet
     Load frmSet
     .lstPlugs.AddItem (.lstPlugs.ListCount + 1) & ". " & File.version
     .lstPlugs.AddItem (.lstPlugs.ListCount + 1) & ". " & Vis.version
...
End Sub
Public Sub LoadfrmPro()

    With frmPro
     Load frmPro
     Call DisableForms(False)
     Call LoadTracks
     Call Graph.Ontop(.hwnd, CI.bTop): PL.Ct = 0
     .Show
    End With

End Sub
Public Sub LoadfrmSkn()
...
Public Sub LoadfrmID3(DoIt As Boolean)
    ...
    With frmID3
     ...
     Call DisableForms(False)
     Call FullName(False)
     If DoIt Then Call GetIt(GL.sTrack)
     Call Graph.Ontop(.hwnd, CI.bTop)
     .Show
    End With
...
Public Sub CheckOnEnd()
    ...
    With frmPl.l.ListItems
     ...
     If UBound(PL.Tr) <> 0 And PL.Ct <> 0 And PL.Tr(PL.TrS) <= .Count Then
      Call CheckProgram
     Else
      If GL.lPos = .Count Then
       If CI.bRand = False And CI.bLoop Then
        Call SetScroller(1)
        Call GetPlay(True)
       End If
