    ...
    With frmPl.l.ListItems
     ...
     Else
      For i = 0 To F.ListCount - 1
       ...
       .Item(.Count).Tag = Ini.getshortpath(Path & F.List(i))
      Next i
     End If
...
Public Sub ToTime(Dur As Long)
    ...
    With frmTim.txtTime
     ...
     If MP.FileName <> "" Or File.ConvertMinSec(.Text) < Dur _
     Or .Text <> "00:00" Or Left(.Text, 3) = ":" Then
      Call File.ConvertMinSec(.Text)
      MP.CurrentPosition = File.ConvertMinSec(.Text)
     Else
      .Text = "00:00"
     End If
...
End Sub
Public Sub SortList(l As MSComctlLib.ListView, SortKey As Integer)
...
End Sub
Public Sub ShowVis(D As Boolean)

    With frmMn
     If .WindowState <> vbNormal Then Exit Sub
     If MP.playstate = 3 Then Exit Sub
     .tmrVis.Enabled = D
     .Scope.Visible = D
    End With
...
Public Function TimePosition(Time As String, Optional DoIt As Boolean) As String
    ...
    If DoIt Then Call SetNumbers(CInt(Left(Min, 1)), CInt(Right(Min, 1)), CInt(Left(Format(Sec, "00"), 1)), CInt(Right(Format(Sec, "00"), 1)))
