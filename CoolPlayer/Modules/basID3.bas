...
Public Sub CreateKeys()
    ...
    Call SaveSetting("CoolPlayer", "Plugins", "Registered", "True")
    Call Register(frmMn.hwnd, App.Path & "\Plugins\Misc_v1.dll", True)
    Call Register(frmMn.hwnd, App.Path & "\Plugins\visOut.dll", True)
...
Public Sub DeleteKeys()
    ...
    Call Register(frmMn.hwnd, App.Path & "\Plugins\visOut.dll", False)
    If Len(GetSetting("CoolPlayer", "Plugins", "Registered")) <> 0 Then
     Call DeleteSetting("CoolPlayer")
    End If
...
Public Sub CheckSort(Check As Boolean)
    If Check Then Call SortList(frmPl.l, 0)
End Sub
